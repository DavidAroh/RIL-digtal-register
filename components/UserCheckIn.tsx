...
export default function UserCheckIn() {
  ...
  const handleLogin = async () => {
    ...
    setIsLoading(true);
    try {
      const user = validateOTP(email, otp);
      if (user) {
        const todayStatus = getUserTodayStatus(user.id);
        const userSession: UserSession = {
          userId: user.id,
          email: user.email,
          name: user.name,
          isCheckedIn: todayStatus.isCheckedIn,
          lastCheckIn: todayStatus.lastAction?.timestamp,
        };
        saveUserSession(userSession);
        setSession(userSession);
        setStep(3); // Show success confirmation screen
        setTimeout(() => {
          setStep(1); // Go back to login page or can go straight to dashboard if you want
        }, 2000);
        setEmail("");
        setOtp("");
      } else {
  ...
  const handleCheckInOut = async () => {
    ...
    setIsLoading(true);
    try {
      const action = session.isCheckedIn ? "check-out" : "check-in";
      addCheckInRecord({
      ...
      setMessage({ type: "error", text: "Check-in/out failed" });
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <Card className="shadow-xl">
            ...
            <CardContent className="space-y-6">
              ...
              <Button
                ...
                disabled={isLoading}
              >
                {session.isCheckedIn ? "Check Out" : "Check In"}
              </Button>
              <Button
